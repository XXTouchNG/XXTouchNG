...
function alert_helper_test(step)
    ...
    if step == nil or step == "1-2" then
        nLog("#1-2 关闭自动点击")
        alert.disable_auto_bypass()
    end
    
    
    if step == nil or step == "2-1" then
        nLog("#2-1 设置全局规则")
        alert.set_global_dialog_rules({
                {
        ...
        nLog(alert.get_global_dialog_rules())
    end
    ...
    if step == nil or step == "2-2" then
        nLog("#2-2 设置应用本地规则")
        alert.disable_auto_bypass()
        alert.clear_local_dialog_rules("com.apple.mobilesafari")  -- 使用专门的清空函数, 而不设置空表
        alert.set_local_dialog_rules("com.apple.mobilesafari", {
                {
                    ["title"] = "^Log in to",
                    --["textfields"] = {
                    --    { ["text"] = "user" },
        ...
        alert.clear_local_dialog_rules("*")  -- 使用 '*' 代表清空所有应用的本地规则 (不清空全局规则)
        nLog(alert.get_local_dialog_rules("com.apple.mobilesafari"))
    end
    
    
    if step == nil or step == "2-3" then
        nLog("#2-3 测试自动点击")
        alert.enable_auto_bypass()
        alert.set_auto_bypass_delay(500.0)  -- 设置自动点击延迟 (毫秒, 若小于 100 毫秒则视为 100 毫秒)
    ...
    if step == nil or step == "2-4" then
        nLog("#2-3 测试实时点击")
        local topMostDialog, err = alert.get_top_most_dialog()
        nLog(topMostDialog, err)
        if topMostDialog ~= nil then
            nLog(alert.dismiss_top_most_dialog())  -- 默认点击第一个按钮
            --nLog(alert.dismiss_top_most_dialog(1))  -- 通过索引指定动作 (以 1 为起始, 0 的效果和 1 一致)
            --nLog(alert.dismiss_top_most_dialog("^OK"))  -- 通过标题指定动作 (正则表达式)
            --nLog(alert.dismiss_top_most_dialog({
            --            ["title"] = "^OK",  -- 通过标题指定动作 (正则表达式)
            --            ["delay"] = 1000.0,  -- 设置动作延迟 (毫秒)
            --}))
            --nLog(alert.dismiss_top_most_dialog({ ["action"] = {
            --            ["title"] = "^OK",  -- 通过标题指定动作 (正则表达式)
            --            ["delay"] = 1000.0,  -- 设置动作延迟 (毫秒)
            --}}))
        end
