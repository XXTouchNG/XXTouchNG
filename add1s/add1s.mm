...
#import <Menes/Function.h>
#import <Foundation/Foundation.h>

typedef Function<void, const char *, launch_data_t> LaunchDataIterator;

static void launch_data_dict_iterate(launch_data_t data, LaunchDataIterator code) {
...
int main(int argc, char *argv[]) {
    ...
    NSCAssert(response != NULL, @"launch_msg() failed");
    NSCAssert(launch_data_get_type(response) == LAUNCH_DATA_DICTIONARY, @"launch_msg() failed");
    ...
    struct stat correct;
    if (lstat("/Applications/XXTExplorer.app/XXTExplorer", &correct) == -1) {
    ...
    boolean_t is_dpkg(false);
    if (argc < 2 || argv[1][0] != '/') {
        argv[0] = strdup("/usr/bin/dpkg");
        is_dpkg = true;
    } else {
        --argc;
        ++argv;
