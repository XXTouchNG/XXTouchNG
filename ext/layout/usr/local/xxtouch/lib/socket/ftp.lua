...
function metat.__index:rest(pos)    
    self.try(self.tp:command("rest", pos))
    self.try(self.tp:check(350))
    return 1
end

function metat.__index:mkd(dir)    
    self.try(self.tp:command("mkd", dir))
    self.try(self.tp:check(257))
    return 1
end
...
function metat.__index:rmd(dir)
    self.try(self.tp:command("rmd", dir))
    self.try(self.tp:check(250))
    return 1
end

function metat.__index:size(file)
    self.try(self.tp:command("size", file))
    local _, reply = self.try(self.tp:check(213))
    return tonumber(string.match(reply, "%d+ (%d+)"))
end
...
local function tget(gett)
    ...
    if gett.rest then f:rest(gett.rest) end  --added 6/15/2022
    f:receive(gett)
    f:quit()
    return f:close()
end
...
_M.mkdir = socket.protect(function(cmdt)
    cmdt = override(cmdt)
    socket.try(cmdt.host, "missing hostname")
    socket.try(cmdt.path, "missing path")
...
_M.rmdir = socket.protect(function(cmdt)
    cmdt = override(cmdt)
    socket.try(cmdt.host, "missing hostname")
    socket.try(cmdt.path, "missing path")
    local f = _M.open(cmdt.host, cmdt.port, cmdt.create)
...
_M.size = socket.protect(function(cmdt)
    cmdt = override(cmdt)
    socket.try(cmdt.host, "missing hostname")
    socket.try(cmdt.path, "missing path")
    local f = _M.open(cmdt.host, cmdt.port, cmdt.create)
